...
- `client_secret` (String, Optional, Sensitive)
  GitHub OAuth client secret. Required when creating the resource. On later updates, it may be
...
- `authorization_url` (String, Optional)
  Authorization endpoint URL. For GitHub, the API may default this value.
- `token_url` (String, Optional)
  Token endpoint URL. For GitHub, the API may default this value.
- `userinfo_url` (String, Optional)
  Userinfo endpoint URL. For GitHub, the API may default this value.
...
```bash
terraform import neuwerk_sso_provider_github.github sso-123
```

## Notes

- The API only reports whether `client_secret` is configured, not the raw secret value.
- GitHub endpoint URLs may be defaulted by the API and then read back into Terraform state even if
